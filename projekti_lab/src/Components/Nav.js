  ...
  return (
    <div>
      ...
      <nav className="navbar row navbar-light mx-5 mt-2 bg-secondary" style={{width: "12%", position: "absolute", height: "80%", alignContent: 'flex-start'}}>
        {usertoken && <Link to="/home" onClick={() => {window.location.href="/home"}} className="btn btn-secondary btn-lg rounded-0 text-start"><MdHome/> Home</Link>}
      {usertoken && (checkAdmin() || checkInfermier()) && <Link to="/dhomat" onClick={() => {window.location.href="/dhomat"}} className="btn btn-secondary btn-lg rounded-0 text-start"><MdMeetingRoom/> Dhomat</Link>}
        ...
        {usertoken && checkAdmin() && <Link to="/activitylog" onClick={() => {window.location.href="/activitylog"}} className="btn btn-secondary btn-lg w-15 rounded-0 text-start"> <BsFillEyeFill/> Activity Log</Link>}
